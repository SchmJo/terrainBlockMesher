...
bool TerrainManager::calc() {
	...
	// modify orography:
	if(modificationModule_.ready() && !modificationModule_.calc()){
		Info << "\n   TerrainManager: Error during orography modification." << endl;
		return false;
	}

	// calculate outer cylinder
	if(cylinderModule_.ready() && !cylinderModule_.calc()){
...
bool TerrainManager::calcUpSplines(){
	...
	// disable if cylinder module is selected:
	if(cylinderModule_.ready()){
		Info << "   warning: calcOrthogonalUpwardSplines disabled in cylinder mode. Ignoring request." << endl;
		return true;
	}
